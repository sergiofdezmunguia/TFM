...
class UNetGenerator(nn.Module):
    def __init__(self, in_channels=3, out_channels=1, features=32):
        ...
        self.bottleneck = self._unet_block(features * 8, features * 8, batch_norm=False, activation="leaky", stride_override=bottleneck_stride)
        ...
        self.dec4 = self._unet_block(features * 8 * 2, features * 4, batch_norm=True, activation="relu", upsample=True)
        self.dec5 = self._unet_block(features * 4 * 2, features * 2, batch_norm=True, activation="relu", upsample=True)
        self.dec6 = self._unet_block(features * 2 * 2, features,     batch_norm=True, activation="relu", upsample=True)
    ...
    def _unet_block(self, in_channels, out_channels, batch_norm=True, activation="relu", 
        ...
        if upsample:
            ...
            layers.append(nn.Conv2d(in_channels, out_channels, kernel_size=4, stride=stride, padding=1, bias=False))
    ...
    def forward(self, x):
        ...
        final_input = torch.cat([d6_out, e2], dim=1)
        
        final_input_to_last_layer = torch.cat([d6_out, e1], dim=1)
        out = self.final_conv_out(final_input_to_last_layer)
